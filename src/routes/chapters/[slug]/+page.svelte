<script lang="ts">
	...
	function getSectionClass(section: string) {
		if (section === 'desert') {
			return 'text-desert-800 hover:text-desert-900 dark:text-desert-100 dark:hover:text-desert-200';
		} else if (section === 'sierras') {
			return 'text-sierras-700 hover:text-sierras-800 dark:text-sierras-800';
		} else if (section === 'norcal') {
			return 'text-norcal-700 hover:text-norcal-800 dark:text-norcal-800';
		} else if (section === 'cascades') {
			return 'text-cascades-600 hover:text-cascades-800 dark:text-cascades-300';
		} else if (section === 'end') {
			return 'text-end-400 hover:text-end-500 dark:text-end-300';
		}
	}
</script>
...
<nav class="flex items-center justify-between lg:px-32 mt-6 font-display" data-sveltekit-reload>
	{#if data.previousChapter}
		<div class="-mt-px flex w-0 flex-1">
			<a class={getSectionClass(data.previousChapterSection)} href="/chapters/{data.previousChapter}">Previous</a>
		</div>
	{/if}
	{#if data.nextChapter}
		<div class="-mt-px flex w-0 flex-1 justify-end cursor-pointer">
			<a class={getSectionClass(data.nextChapterSection)} href="/chapters/{data.nextChapter}">Next</a>
		</div>
	{/if}
</nav>
